...
public class PlayerService {
    ...
    @Nonnull
    private Predicate<SessionEntity> olderThanADay() {
        return sessionTimeCalculator::olderThanADay;
    }
    ...
    private SessionEntity newSession(PlayerEntity playerEntity) {
        return SessionEntity.builder().player(playerEntity)
                            .createdAt(Instant.now())
                            .activeTimeInSeconds(0)
                            .lastLoginAt(Instant.now()).build();
    }
    ...
    public void logoutPlayer(UUID sessionId) {
        ...
        long duration = Duration.between(session.getLastLoginAt(), session.getLastLogoutAt()).get(ChronoUnit.SECONDS);
        var activeTime = session.getActiveTimeInSeconds() + duration;
